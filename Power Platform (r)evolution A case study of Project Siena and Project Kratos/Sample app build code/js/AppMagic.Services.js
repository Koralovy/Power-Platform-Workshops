    ...
    (function(Services) {
        var AzureGraphConnection = function() {
                ...
                return AzureGraphConnection.prototype.addApplicationAsync = function(name, redirectUri) {
                        ...
                        return app.availableToOtherTenants = !1, app.displayName = name, app.homepage = redirectUri, app.publicClient = !0, app._replyUrls.push(redirectUri), app._replyUrls.push(Services.AzureConstants.Graph.sienaSidRedirectUri), app._replyUrlsChanged = !0, this._addGraphResourceAccessAsync(app).then(function() {
                                    return _this._addExchangeResourceAccessAsync(app)
                                }).then(function() {
                                    return _this._addSharePointResourceAccessAsync(app)
                                }).then(function() {
                                    return _this._addGraphObjectAsync(app, Services.AzureConstants.Graph.Routes.applications)
                                }, function(error) {
                        ...
                        return sp.accountEnabled = !0, sp.displayName = appName, sp.appId = appId, isAuthoringApp ? (sp._tags.push(Services.AzureConstants.Graph.clientAppTag_AuthoringAppGuid), sp._tags.push(Services.AzureConstants.Graph.clientAppTag_AuthoringAppVersion)) : (sp._tags.push(Services.AzureConstants.Graph.clientAppTag_PublishedAppGuid), sp._tags.push(Services.AzureConstants.Graph.clientAppTag_PublishedAppVersion)), sp._tagsChanged = !0, this._addGraphObjectAsync(sp, Services.AzureConstants.Graph.Routes.servicePrincipals)
                        ...
                        }, AzureGraphConnection.prototype._addGraphResourceAccessAsync = function(app) {
                            var _this = this;
                            return this.getServicePrincipalAsync(AzureGraphConnection.graphResourceAppId).then(function(graphServicePrincipal) {
                                    if (graphServicePrincipal === null)
                                        return WinJS.Promise.wrap(null);
                                    var graphPermissionIds = graphServicePrincipal.appPermissions.filter(function(permission) {
                                            return AzureGraphConnection.graphPermissions.indexOf(permission.claimValue) >= 0
                                        }).map(function(permission) {
                                            return permission.permissionId
                                        }),
                                        graphResourceAccess = _this._generateRequiredResourceAccess(AzureGraphConnection.graphResourceAppId, graphPermissionIds);
                                    app.requiredResourceAccess.push(graphResourceAccess)
                                })
                        }, AzureGraphConnection.prototype._addExchangeResourceAccessAsync = function(app) {
                            var _this = this;
                            return this.getServicePrincipalAsync(AzureGraphConnection.exchangeResourceAppId).then(function(exchangeServicePrincipal) {
                                    if (exchangeServicePrincipal === null)
                                        return WinJS.Promise.wrap(null);
                                    var exchangePermissionIds = exchangeServicePrincipal.appPermissions.filter(function(permission) {
                                            return AzureGraphConnection.exchangePermissions.indexOf(permission.claimValue) >= 0
                                        }).map(function(permission) {
                                            return permission.permissionId
                                        }),
                                        exchangeResourceAccess = _this._generateRequiredResourceAccess(AzureGraphConnection.exchangeResourceAppId, exchangePermissionIds);
                            ...
                            var _this = this;
                            return this.getServicePrincipalAsync(AzureGraphConnection.sharepointResourceAppId).then(function(sharePointServicePrincipal) {
                                    if (sharePointServicePrincipal === null)
                                        return WinJS.Promise.wrap(null);
                                    var sharePointPermissionIds = sharePointServicePrincipal.appPermissions.filter(function(permission) {
                                            return AzureGraphConnection.sharePointPermissions.indexOf(permission.claimValue) >= 0
                                        }).map(function(permission) {
                                            return permission.permissionId
                                        }),
                                        sharePointResourceAccess = _this._generateRequiredResourceAccess(AzureGraphConnection.sharepointResourceAppId, sharePointPermissionIds);
                        ...
                        }, AzureGraphConnection.appIdFilterFormat = "appId eq guid'{0}'", AzureGraphConnection.graphResourceAppId = "00000002-0000-0000-c000-000000000000", AzureGraphConnection.exchangeResourceAppId = "00000002-0000-0ff1-ce00-000000000000", AzureGraphConnection.sharepointResourceAppId = "00000003-0000-0ff1-ce00-000000000000", AzureGraphConnection.graphPermissions = ["user_impersonation"], AzureGraphConnection.exchangePermissions = ["Mail.Send", "Mail.Write", "Contacts.Write", "Calendars.Write"], AzureGraphConnection.sharePointPermissions = ["AllSites.Write"], AzureGraphConnection.pageSize = 999, AzureGraphConnection.maxRetrievalAttempts = 15, AzureGraphConnection
            }();
